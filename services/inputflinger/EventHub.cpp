...
size_t EventHub::getEvents(int timeoutMillis, RawEvent* buffer, size_t bufferSize) {
    ...
    for (;;) {
        ...
        if(samp.total_events) {
			...
			int err;
			ALOGV(" id =%d, type=%d, code=%d, x=%d, y=%d, press=%d, value=%d", deviceId, samp.ev[numOfEventsSent].type, 
				samp.ev[numOfEventsSent].code, samp.x, samp.y, samp.pressure, samp.ev[numOfEventsSent].value);
            event->deviceId = deviceId;
